...
% "Change" The examples take the form of rankings that state that one possible world is preferred over another one.
...
%"Expand something here yet?" ...%Each example ....
...
% "Unify usage of examples / preferences" Therefore, the examples will take the form of rankings, where each ranking specifies that one particular possible world is preferred over another one.
...
Moreover, it differs the SRL and MLN approaches in that one learns from preferences rather than from examples.
 % and rankings are used rather than possible worlds + the target value of the examples, and differs from the SRL and MLN approaches in that one learns from rankings rather than from examples. 
...
% "Rewrite" In Section 2, we shall introduce the formalism and the setting,
% "Rewrite" in Section 3, the algorithm and system X to solve this problem, in Section 4, 
% "Rewrite" we shall report on some experiments, and Finally, in Section 5 ,we conclude
...
% \paragraph{Logic Solver} " v ------ v   Dissolve"
