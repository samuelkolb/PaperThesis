...
\sam{it might be good to state somewhere early that you are in the "learning from interpretations" setting, but additionally have preferences}
\sam{Structure}
...
An atom $p(t_1, t_2, ..., t_n)$ consists of a predicate symbol~$p$ that is applied to $n$ terms~$t_i$. \sam{Casing}
...
\begin{example}[Moving]
  ...
  Consider the set~\sym{S} of shared ground atoms $\{\mathit{cheap}(A_1)$, $\mathit{cheap}(A_3)$, $\mathit{low\_crime}(A_1)$, $\mathit{low\_crime}(A_2)\}$ and define the examples as:
  \begin{flalign*}
    &e_1 = \{\mathit{live\_in}(A_1), \mathit{work\_in}(A_2), \mathit{school\_in(A_2)}\} \cup \sym{S} \\
    &e_2 = \{\mathit{live\_in}(A_2), \mathit{work\_in}(A_2), \mathit{school\_in(A_3)}\} \cup \sym{S}\\
    &e_3 = \{\mathit{live\_in}(A_2), \mathit{work\_in}(A_2), \mathit{school\_in(A_1)}\} \cup \sym{S}.
  \end{flalign*}
